...
W_R5_GAP = 10  # NEW: penalty for skipping the clean-down gap (R-5 soft)
...
def classify_complexity(row):
    """R-6 complexity proxy for testing. Adjust thresholds if ops refine them."""
...
if "IS_EASY" in df.columns and "IS_HARD" in df.columns:
    df["IS_EASY"] = df["IS_EASY"].fillna(0).astype(int)
    df["IS_HARD"] = df["IS_HARD"].fillna(0).astype(int)
else:
...
for idx, row in df.iterrows():
    ...
    model.Add(
        too_early >= win_low - (start_mod + duration)
    )  # finish before window start
    model.Add(too_late >= start_mod - win_high)  # start after window end
    ...
    # -------- R-2: board arrival (HARD via big penalty to early start) --------
    ...
    # -------- R-3: freeze rule (HARD <=5h, SOFT <=24h) --------
    ...
    if pd.notna(plan_start):
        ...
        if 0 <= delta_to_plan_s <= 5 * 3600:
            model.Add(start == planned_min)  # hard lock
    ...
    # -------- R-5: clean-down gap (SOFT via optional interval) --------
    if colcount is not None and colcount >= 5:
        # Create an optional gap interval right after this job.
        gap_present = model.NewBoolVar(f"gap_present_{job_id}")
        # link start/end of the gap to job end
        gap_start = model.NewIntVar(0, H_MIN, f"gap_start_{job_id}")
        gap_end = model.NewIntVar(0, H_MIN, f"gap_end_{job_id}")
        model.Add(gap_start == end)
        model.Add(gap_end == gap_start + R5_GAP_MINUTES)
        gap_interval = model.NewOptionalIntervalVar(
            gap_start, R5_GAP_MINUTES, gap_end, gap_present, f"gap_{job_id}"
        )
        machine_to_intervals[machine].append(gap_interval)

        # Either take the gap OR pay a penalty.
        skip_gap = model.NewBoolVar(f"skip_gap_{job_id}")
        model.Add(gap_present + skip_gap == 1)
        r5_skip_gap_bools.append(skip_gap)
...
# R-5 skip-gap penalties (sum of booleans)
r5_skip_pen = safe_sum(model, r5_skip_gap_bools, "r5_skip_pen")
...
if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    print(
        ...
        f" r5_skip={solver.Value(r5_skip_pen)}"
    )
    ...
    out.to_excel("schedule_output_r1to6_soft.xlsx", index=False)
    print("📁 Schedule saved to schedule_output_r1to6_soft.xlsx")
else:
